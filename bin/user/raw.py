...
class RawService(StdService):
	...
	def redisPublish(self, packet):
		if self.redisPublishEnabled:
			...
			try:
				pub = self.publisher.publish(channel, rawWxData)
			except Exception, e:
	...
	def setupRawDatabase(self, config_dict):
		...
		self.rawData = RawData.open_with_create(config_dict['Databases'][raw_db], 
	...
	def redisConnect(self, config_dict):
		if self.redisPublishEnabled:
			...
			try:
				...
				syslog.syslog(syslog.LOG_INFO, "Raw: Connecting to redis server: %s:%s " % (host, port))
				self.publisher = redis.Redis(host=host, port=port, db=0)
			except Exception, e:
				syslog.syslog(syslog.LOG_ERR, "Raw: unable to connect to redis server. Reason: %s" % e)
